...
def create_runtime(meeting_id: UUID, request: Request) -> dict[str, object]:
    return _service(request).start(meeting_id).as_dict()
...
def stop_runtime(runtime_session_id: UUID, request: Request) -> dict[str, object]:
    session = _service(request).stop(runtime_session_id)
    if session is None:
        raise HTTPException(status_code=404, detail="runtime not found")
    return session.as_dict()
