...
speaker_store = QdrantSpeakerStore(settings.speaker_database_path)
...
def _all_speaker_points(*, with_vectors: bool = False):
    # Compatibility helper retained while speaker policy remains in this
    # module; persistence itself is isolated behind QdrantSpeakerStore.
    return speaker_store.all_points(with_vectors=with_vectors)
...
def _query_speaker_scores(embedding: np.ndarray) -> dict[str, float]:
    return speaker_store.query_scores(embedding)
...
room_timeline = CoordinatedVadTimeline(
    ...
    final_wait_for_turn_close_seconds=(
        settings.timeline_final_wait_for_turn_close_seconds
    ),
)
...
session_manager = SessionManager(_assignment_state_store)
...
async def create_ai_session(
    ...
    try:
        state, names = session_manager.create(payload, idempotency_key)
    except SessionConflict as exc:
        status_code = 409 if str(exc) == "another AI session is active" else 422
...
async def get_ai_session(
    ...
    _require_service_key(x_internal_key, x_service_key)
    state = session_manager.state(runtime_session_id)
    if state is None:
...
async def stop_ai_session(
    ...
    _require_service_key(x_internal_key, x_service_key)
    state = session_manager.stop(runtime_session_id, idempotency_key)
    if state is None:
        raise HTTPException(status_code=404, detail="AI session not found")
    return state
...
async def update_ai_participants(
    ...
    _require_service_key(x_internal_key, x_service_key)
    try:
        state, names = session_manager.update_participants(runtime_session_id, payload)
    except SessionConflict as exc:
...
async def get_agent_assignment(
    ...
    _require_service_key(x_internal_key, x_service_key)
    return session_manager.assignment(after_generation, settings.livekit_url)
...
async def update_agent_status(
    ...
    _require_service_key(x_internal_key, x_service_key)
    return {"status": session_manager.update_agent(payload)}
...
async def websocket_endpoint(
    ...
    if runtime_session_id:
        if not session_manager.validate_websocket(runtime_session_id, meeting_id):
    ...
    async def process_final_minute_bg(
        ...
        if audio_snapshot:
            ...
            enrolled_points = speaker_store.count()
        ...
        try:
            async for evt in vad_stream:
                if evt.type == VADEventType.START_OF_SPEECH:
                    ...
                    room_timeline.finalization_started(
                        current_turn_id, identity
                    )
...
def run_server() -> None:
    """Run the Meeting AI HTTP/WebSocket adapter.

    Kept as a function so the legacy ``ai_server.py`` entrypoint can delegate
    without importing Uvicorn during normal package imports.
    """
    import os
    import uvicorn
